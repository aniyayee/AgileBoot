...
import com.github.core.config.AgileBootConfig;
...
public class FileUploadUtils {
     ...
     * 默认大小 50M
     */
    public static final long MAX_FILE_SIZE = 50 * MB;
    ...
    public static String upload(String subDir, MultipartFile file) {
        try {
            ...
            throw new AgileException(ErrorCode.Business.UPLOAD_FILE_FAILED, e.getMessage());
        }
    }
    ...
    static void saveFileToLocal(MultipartFile file, String subDir, String fileName) throws IOException {
        if (StrUtil.isEmpty(subDir) || StrUtil.isEmpty(fileName)) {
            throw new AgileException(ErrorCode.Internal.INVALID_PARAMETER, "subDir or fileName");
        }
    ...
    static String getRelativeFileUrl(String subDir, String fileName) {
        ...
        return StrUtil.format("/{}/{}/{}", SystemConfigConstant.RESOURCE_PREFIX, subDir, fileName);
    }
    ...
    static void isAllowedUpload(MultipartFile file, String[] allowedExtension) {
        ...
        if (fileNameLength > MAX_FILE_NAME_LENGTH) {
            throw new AgileException(ErrorCode.Business.UPLOAD_FILE_NAME_EXCEED_MAX_LENGTH, MAX_FILE_NAME_LENGTH);
        }

        long size = file.getSize();
        if (size > MAX_FILE_SIZE) {
            throw new AgileException(ErrorCode.Business.UPLOAD_FILE_SIZE_EXCEED_MAX_SIZE, MAX_FILE_SIZE / MB);
        }

        String extension = getFileExtension(file);
        if (!isExtensionAllowed(extension, allowedExtension)) {
            throw new AgileException(ErrorCode.Business.UPLOAD_FILE_TYPE_NOT_ALLOWED,
